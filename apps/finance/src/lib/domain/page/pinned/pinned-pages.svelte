...
<section class="flex flex-col gap-3 md:mb-4">
    ...
    >
        ...
        {:else if $pinnedPagesQuery.data}
            {#each $pinnedPagesQuery.data as { page }}
                {#if page}
                    <button
                        class="flex w-60 shrink-0 flex-col gap-1 rounded-lg border border-surface-200 bg-white p-3 transition-colors duration-100 hover:border-primary-300"
